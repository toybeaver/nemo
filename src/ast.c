...
struct ast_node parse_ast(const char* src, struct lex_token* tokens)
{
  ...
  while (cur->type != TOKEN_EOF) {
    if (is_func(src, cur)) parse_func(src, &cur, &root);
      ...
      fprintf(stderr, "error: [0]Unexpected token found on root scope: \"%s\" at pos %zu\n", s, cur->pos);
      exit(1);
    }
  }
...
static void consume_single_token(const char* src, struct lex_token **_cur, TokenType type, char expected)
{
  ...
  if (cur->type != type) {
    ...
    fprintf(stderr, "error: Expected '%c' found \"%s\" at pos %zu\n", expected, bad_token, cur->pos);
    exit(1);
  }
  *_cur = cur+1;
}
...
static char* consume_ident(const char* src, struct lex_token** _cur, struct ast_node* parent, bool uniq)
{
  ...
  if (uniq && hm_get(parent->sym_table->table, ident_name) != NULL) {
    fprintf(stderr, "error: Unique symbol expected, but second declaration found: \"%s\" at pos %zu\n", ident_name, cur->pos);
    exit(1);
  }
...
static char* consume_literal(const char* src, struct lex_token **_cur, struct ast_node* parent)
{
  ...
  if (cur->type != TOKEN_LITERAL) {
    fprintf(stderr, "error: [2]Unexpected token \"%s\" at pos %zu\n", lit, cur->pos);
    exit(1);
  }
...
static struct data_type_definition* consume_type(const char* src, struct lex_token **_cur, struct ast_node* parent)
{
    ...
    if (cur->type != TOKEN_IDENT) {
      char* bad_token = get_tokens_content(src, cur);
      fprintf(stderr, "error: Expected type, found \"%s\" at pos %zu\n", bad_token, cur->pos);
      exit(1);
    }
    ...
    if (dt == NULL) {
      fprintf(stderr, "error: \"%s\" at pos %zu is not a valid type\n", type_name, cur->pos);
      exit(1);
    }
...
static void parse_scope(const char* src, struct lex_token** _cur, struct ast_node* parent)
{
  ...
  while(true) {
    ...
    switch (cur->type) {
      case TOKEN_EOF:
        fprintf(stderr, "error: Unexpected EOF in func at pos %zu\n", cur->pos);
        exit(1);
      case TOKEN_RBRACKET:
        cur += 1;
        should_quit = true;
...
static void parse_assign(const char* src, struct lex_token** _cur, struct ast_node* parent)
{
  ...
  if (sym == NULL) {
    fprintf(stderr, "error: Undeclared variable \"%s\"at pos %zu\n", var_name, cur->pos);
  ...
  switch (cur->type) {
    ...
    case TOKEN_EOF: {
      fprintf(stderr, "error: Unexpected EOF at pos %zu\n", cur->pos);
      exit(1);
    }
    default: {
      char* bad_token = get_tokens_content(src, cur);
      fprintf(stderr, "error: [3]Unexpected token \"%s\" at pos %zu\n", bad_token, cur->pos);
      exit(1);
    }
  }
...
static void parse_operand(const char *src, struct lex_token **_cur, struct ast_node *parent)
{
  ...
  switch (cur->type) {
    ...
    default: {
      ...
      fprintf(stderr, "error: [5]Unexpected token \"%s\" at pos %zu\n", bad_token, cur->pos);
      exit(1);
    }
  }
  ...
  switch (cur->type) {
    case TOKEN_LITERAL: {
      if (!is_bool_literal(src, cur)) {
        char* bad_token = consume_literal(src, &cur, parent);
        fprintf(stderr, "error: Unexpected non-bool literal \"%s\" in boolean expression at pos %zu\n", bad_token, cur->pos);
        exit(1);
      }
